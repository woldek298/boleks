                                                ...
                                                fermat_t *found352)
{
  ...
  const unsigned mask320 = __ballot_sync(fullMask, is320);
#else
  const unsigned mask320 = __ballot(is320);
#endif
...
#else
    base320 = __shfl(base320, leader320);
#endif
...
#if defined(__CUDACC_VER_MAJOR__) && (__CUDACC_VER_MAJOR__ >= 9)
  const unsigned mask352 = __ballot_sync(fullMask, is352);
#else
  const unsigned mask352 = __ballot(is352);
#endif
  if (mask352) {
...
#else
    base352 = __shfl(base352, leader352);
#endif
