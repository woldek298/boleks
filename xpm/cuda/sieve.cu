...
__global__ void sieve(uint32_t *__restrict__ gsieve_all,
                      const uint32_t *__restrict__ offset_all,
                      const uint2 *__restrict__ primes)
{
...
__device__ __forceinline__ void appendFoundWarp(bool is320,
                                                ...
                                                fermat_t *found352)
{
  const uint32_t addr = atomicAdd(&fcount[is320 ? 0 : 1], 1);
  if (is320)
    found320[addr] = info;
  else
    found352[addr] = info;
...
__global__ void s_sieve(const uint32_t *__restrict__ gsieve1,
                        const uint32_t *__restrict__ gsieve2,
                        fermat_t *__restrict__ found320,
                        fermat_t *__restrict__ found352,
                        uint32_t *__restrict__ fcount,
                        uint32_t hashid,
                        uint32_t hashSize,
                        uint32_t depth)
{
  ...
  const uint32_t sieveSpanWords = SIZE * STRIPES / 2;

  uint32_t tmp1[WIDTH];
  const uint32_t *g1 = gsieve1 + id;
#pragma unroll
